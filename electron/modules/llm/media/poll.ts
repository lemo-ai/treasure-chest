...
/** Poll an async media job until URL / terminal failure. */
export async function pollUntilUrl(opts: {
  label: string
  intervalMs?: number
  timeoutMs?: number
  fetchStatus: () => Promise<{
  ...
  while (Date.now() - started < timeout) {
    try {
      ...
      if (snap.done && !snap.url) {
        return { ok: false, error: `${opts.label} finished without a media URL` }
      }
    } catch (err) {
