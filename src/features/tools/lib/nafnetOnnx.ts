...
/** OpenCV NAFNet style: RGB float NCHW in [0,1]. */
...
async function denoiseTiled(
  ...
  for (let y = 0; y < sh; y += step) {
    for (let x = 0; x < sw; x += step) {
      ...
      const padX = x0 === 0 ? 0 : OVERLAP
      const padY = y0 === 0 ? 0 : OVERLAP
      const cropW = Math.min(restored.width - padX * 2, x + step > sw ? sw - x : step)
      const cropH = Math.min(restored.height - padY * 2, y + step > sh ? sh - y : step)
      octx.drawImage(restored, padX, padY, cropW, cropH, x, y, cropW, cropH)
    }
  }
  return out
}
...
/**
 * NAFNet / generic NCHW denoise-deblur ONNX (OpenCV-style float RGB [0,1]).
 * Tiles large images for WASM memory safety.
 */
export async function denoiseWithNafnet(
  imageDataUrl: string,
  ...
  const restored =
    sw <= TILE && sh <= TILE ? await runTile(session, source) : await denoiseTiled(session, source)
